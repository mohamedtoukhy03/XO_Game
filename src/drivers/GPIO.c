...
{
  while (GPIO_PORTS_REGISTERS_ARR[F]->MIS!=0)
  {
    if (GPIORIS(F,4)) /* is it SW1 (PF4)?*/
    {                            /* GPIOF4 pin interrupt */
      /* turn ON green LED (PD3) */
      //Delay(100);
      if (!(GPIO_PORTS_REGISTERS_ARR[F]->DATA & 0x10))
      { 
        sw1 = 1;
				sw2=0;
        Interrupt_acknowledge(F,4);
      }
    }
    else if (GPIORIS(F,0)) /* then it must be SW2 (PF0) */
    {                                 /* GPIOF0 pin interrupt */
      /* turn ON blue LED (PD3) */
           Delay(0.2);
      if (!(GPIO_PORTS_REGISTERS_ARR[F]->DATA & 0x01))
      {
        sw2 = 1;
				sw1=0;
        Interrupt_acknowledge(F,0);
      }
    }
    else
    { /* We should never get here */
      /* But if we do, clear all pending interrupts */
      GPIO_PORTS_REGISTERS_ARR[F]->ICR=GPIO_PORTS_REGISTERS_ARR[F]->MIS;
